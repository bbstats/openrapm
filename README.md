...
Out of season it predicts held-out games at 110.45 points per 100 (K=3, the block's own length; the
...
the offensive prior sees the player's weight, binned (FINDINGS 26 added height and weight to both sides; the defensive prior under-rated
players 6-10 and taller by a third of a point per 100 in every era; the fine pair would name the player), and the offensive prior sees his own past on-court record -- his discounted APM over the windows before the block and the possessions behind it, on pair rows so nothing of the target is in the feature (FINDINGS 28).  A kitchen-sink BorutaShap over 111 candidates (FINDINGS 29) then pruned both lists to what carries weight: 31 offensive names and 11 defensive.
