...
def _shooter_chain(S: dict, cfg) -> None:
    """Every system from the shooter-level targets on, registered into `S` in place.

    This was the body of a single `try: ... except ImportError: pass` inside `registry` -- 880 lines
    and twenty imports under a guard written for the one on its first line.  Any ImportError anywhere
    in it truncated the registry at that point in silence, so a system defined below the failure came
    back "unknown" with no reason given.  As its own function the guard can be tight: the caller
    catches ImportError around this call and says what was lost, and a failure in any of the other
    nineteen imports is a broken install, which should raise rather than quietly return half a
    registry.  `S` is mutated in place; the return value is nothing.
    """
    from . import xshoot
...
def registry(cfg, rankmap=None, calmap=None) -> dict:
    ...
    try:                                                   # the shooter-level targets, once built
        from . import xshoot                               # noqa: F401  -- a probe, nothing else
    except ImportError as e:
        # The guard is around the probe ALONE.  It was written for this one import ("once built")
        # and grew to cover twenty over 880 lines, so an ImportError anywhere in the chain
        # truncated the registry in silence and the system you asked for came back "unknown" with
        # no reason.  Now: xshoot missing is a warning and a short registry; anything else failing
        # to import is a broken install and raises, because half a registry is worse than a stack
        # trace that names the problem.
